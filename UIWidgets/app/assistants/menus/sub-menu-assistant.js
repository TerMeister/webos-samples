...
SubMenuAssistant.prototype.setup = function(){
	this.viewMenuModel = { label: $L('View Menu Demo'), 
								items: [{label: $L('SubMenu Button'), submenu:'category-menu'}
									]};		
	...
	Mojo.Event.listen($('toggle_menu_button'),Mojo.Event.tap, this.toggleMenu.bind(this))
...
SubMenuAssistant.prototype.handleCommand = function(event) {
	if(event.type == Mojo.Event.command) {
		...
		{
			case 'cat-all':
				this.controller.get('message').update('All selected')
			break;
			case 'cat-business':
				this.controller.get('message').update('Business selected')
			break;
			case 'cat-personal':
				this.controller.get('message').update('Personal selected')
			break;
			case 'cat-unfiled':
