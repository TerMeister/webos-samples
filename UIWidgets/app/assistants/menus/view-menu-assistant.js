...
ViewMenuAssistant.prototype.handleCommand = function(event) {
	if(event.type == Mojo.Event.command) {
		...
		{
			case 'button-1':
				this.controller.get('message').update('menu button 1 pressed')
			break;
			case 'deadapp':
				this.controller.get('message').update('Dead app pressed')
			break;
			case 'demo':
				this.controller.get('message').update('Demo button selected')
			break;
			default:
...
ButtonAssistant.prototype.activate = function(event) {
	/* put in event handlers here that should only be in effect when this scene is active. For
	   example, key handlers that are observing the document */
}


ButtonAssistant.prototype.deactivate = function(event) {
	/* remove any event handlers you added in activate and do any other cleanup that should happen before
	   this scene is popped or another scene is pushed on top */
}

ButtonAssistant.prototype.cleanup = function(event) {
